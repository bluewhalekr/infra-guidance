...
class minADE(Metric):
    ...
    def update(
        ...
        pred, target, prob, valid_mask, _ = valid_filter(
            pred, target, prob, valid_mask, None, keep_invalid_final_step
        )
        pred_topk, _ = topk(self.max_guesses, pred, prob)
        if min_criterion == "FDE":
            inds_last = (
                valid_mask
                * torch.arange(1, valid_mask.size(-1) + 1, device=self.device)
            ).argmax(dim=-1)
            ...
            self.sum += (
                (
                    torch.norm(
                        pred_topk[torch.arange(pred.size(0)), inds_best] - target,
                        p=2,
                        dim=-1,
                    )
                    * valid_mask
