...
class ArgoverseV2Dataset(Dataset):
        ...
        if raw_dir is None:
            ...
            if os.path.isdir(self._raw_dir):
                self._raw_file_names = [
                    ...
                    if os.path.isdir(os.path.join(self._raw_dir, name))
                ]
            else:
                self._raw_file_names = []
        else:
        ...
        if processed_dir is None:
            ...
            if os.path.isdir(self._processed_dir):
                self._processed_file_names = [
                    name
                    for name in os.listdir(self._processed_dir)
    ...
    def download(self) -> None:
        ...
        extract_tar(
            path=os.path.join(self.root, f"{self.split}.tar"),
            folder=self.raw_dir,
            mode="r",
        )
        self._raw_file_names = [
            name
            for name in os.listdir(os.path.join(self.raw_dir, self.split))
            if os.path.isdir(os.path.join(self.raw_dir, self.split, name))
        ]
        for raw_file_name in self.raw_file_names:
            shutil.move(
                os.path.join(self.raw_dir, self.split, raw_file_name), self.raw_dir
            )
    ...
    def process(self) -> None:
        for raw_file_name in tqdm(self.raw_file_names):
            ...
            centerlines = {
                lane_segment["id"]: Polyline.from_json_data(lane_segment["centerline"])
            ...
            with open(
                os.path.join(self.processed_dir, f"{raw_file_name}.pkl"), "wb"
            ) as handle:
    ...
    def get_agent_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for track_id, track_df in df.groupby("track_id"):
            ...
            velocity[agent_idx, agent_steps, :2] = torch.from_numpy(
                np.stack(
                    [track_df["velocity_x"].values, track_df["velocity_y"].values],
                    axis=-1,
                )
            ).float()
    ...
    def get_map_features(
        ...
        for lane_segment in map_api.get_scenario_lane_segments():
            ...
            polygon_orientation[lane_segment_idx] = torch.atan2(
                centerline[1, 1] - centerline[0, 1], centerline[1, 0] - centerline[0, 0]
            )
            ...
            polygon_is_intersection[lane_segment_idx] = (
                self._polygon_is_intersections.index(lane_segment.is_intersection)
            )
            ...
            point_height[lane_segment_idx] = torch.cat(
                [left_vectors[:, 2], right_vectors[:, 2], center_vectors[:, 2]], dim=0
            )
        ...
        for crosswalk in map_api.get_scenario_ped_crossings():
            ...
            polygon_position[crosswalk_idx + len(cross_walk_ids)] = end_position[
                : self.dim
            ]
            polygon_orientation[crosswalk_idx] = torch.atan2(
                (end_position - start_position)[1], (end_position - start_position)[0]
            )
            polygon_orientation[crosswalk_idx + len(cross_walk_ids)] = torch.atan2(
                (start_position - end_position)[1], (start_position - end_position)[0]
            )
            ...
            polygon_is_intersection[crosswalk_idx + len(cross_walk_ids)] = (
                self._polygon_is_intersections.index(None)
            )
            ...
            num_centerline_points = (
                math.ceil(
                    torch.norm(end_position - start_position, p=2, dim=-1).item() / 2.0
                )
                + 1
            )
            ...
            point_type[crosswalk_idx] = torch.cat(
                [
                    ...
                    torch.full(
                        (len(right_vectors),), crosswalk_type, dtype=torch.uint8
                    ),
                    torch.full((len(center_vectors),), center_type, dtype=torch.uint8),
                ],
                dim=0,
    ...
    def _download(self) -> None:
        ...
        if (os.path.isdir(self.raw_dir) and len(self.raw_file_names) == len(self)) or (
            os.path.isdir(self.processed_dir)
            and len(self.processed_file_names) == len(self)
        ):
            return
        self._processed_file_names = []
        # self.download()
    ...
    def _process(self) -> None:
        ...
        if os.path.isdir(self.processed_dir) and len(self.processed_file_names) == len(
            self.raw_file_names
        ):
            return
